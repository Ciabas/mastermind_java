...
public class Order {
	...
	public Order(int columnsNumber){
		this.columnSize = columnsNumber;
		this.order = randomFromColorsWithoutRepetition(columnsNumber);
	}
	
	public Color[] randomFromColorsWithoutRepetition(int columnsNumber){
	...
	public boolean compareWith(Color[] comparingOrder){
		for(int i = 0; i < order.length; i++){
			if(!order[i].equals(comparingOrder[i])) return false;
		}
		return true;
	}
	
	public int checkForRightColors(Order comparingOrder, Order rightOrder){
		int counter = 0;
	...
	public boolean[] checkForRightSpots(Order comparingOrder, Order rightOrder){
		boolean[] result = new boolean[comparingOrder.getOrder().length];
		for(int i = 0; i < comparingOrder.getOrder().length; i++){
			if(comparingOrder.getOrder()[i].equals(rightOrder.getOrder()[i])) result[i] = true;
			else result[i] = false;
		}
		return result;
	}
